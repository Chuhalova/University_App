...
<div id='section-in-work-reviewing-page' class='row' style='margin:0 !important;'>
        <form class='col-md-12 col-xs-6' style='display:inline-block;text-align:center;' action="{{ url('/teacher/download-work/'.$sw_o->id) }}" method="GET">
            {{method_field('GET')}}
            @csrf
        ...
        <form id="activate-file-button" class='col-md-6 col-xs-6' style='text-align:center;display:inline-block;' action="{{ url('/teacher/approve-work-file/'.$sw_o->id) }}" method='POST'>
            {{method_field('PATCH')}}
            {{ csrf_field() }}
            <label for="uploaded_work_comment"></label>
            <button type="submit" style="margin:25px auto;" class="btn btn-primary">Ухвалити виконану <br> роботу</button>
        </form>
